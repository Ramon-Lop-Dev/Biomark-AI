...
const getMyNotifications = asyncHandler(async (req, res) => {
  ...
  const result = await notificationsService.getNotifications(usuarioId, {
...
const markNotificationAsRead = asyncHandler(async (req, res) => {
  const usuarioId = req.usuarioId || req.usuario?.id;
  const result = await notificationsService.markAsRead(usuarioId, req.params.id);
  return res.status(200).json(result);
});

const markAllNotificationsAsRead = asyncHandler(async (req, res) => {
  const usuarioId = req.usuarioId || req.usuario?.id;
  const result = await notificationsService.markAllAsRead(usuarioId);
  return res.status(200).json(result);
});
